...
import { ArrowUp, Plus, User, Sparkles, Lightbulb, TrendingUp, Layers } from 'lucide-react';
...
interface EnhancedBottomPromptBarProps {
...
const EnhancedBottomPromptBar: React.FC<EnhancedBottomPromptBarProps> = ({
  onGenerate,
  initialPrompt,
  onPromptUsed,
  onToolOpen,
  ...
  const [toolSuggestions, setToolSuggestions] = useState<ToolRecommendation[]>([]);
  ...
  useEffect(() => {
    const analyzePromptDebounced = async () => {
      ...
      const analysis = await promptAnalyzerService.analyzePrompt(prompt, {
        characterId: selectedCameoId || undefined
      });

      setToolSuggestions(analysis.recommendations.slice(0, 2));
      setPerformanceScore(analysis.performanceScore);
      setShowSuggestions(analysis.recommendations.length > 0);
    };

    const timer = setTimeout(analyzePromptDebounced, 500);
  ...
  const handleSubmit = async () => {
    ...
    if (selectedStylePresetId && userId) {
      const preset = await stylePresetService.getStylePreset(selectedStylePresetId);
      if (preset) {
        finalPrompt = stylePresetService.applyStyleToPrompt(prompt, preset.style_data);
        await stylePresetService.incrementUsageCount(selectedStylePresetId);
      }
    }
    ...
    } else if (platformMode === 'tiktok' || platformMode === 'instagram') {
      currentAspectRatio = AspectRatio.PORTRAIT;
    }
  ...
  const getScoreColor = () => {
    if (performanceScore >= 70) return 'text-green-400';
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 flex justify-center pointer-events-none mb-6">
      ...
      >
        <AnimatePresence>
          {showSuggestions && toolSuggestions.length > 0 && prompt.trim() && (
            <motion.div
            ...
            >
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-semibold text-blue-400 uppercase">Smart Suggestions</span>
                  <div className="ml-auto flex items-center gap-1">
                    <TrendingUp className="w-3 h-3" />
                    <span className={`text-xs font-bold ${getScoreColor()}`}>{performanceScore}%</span>
                  </div>
                </div>
                <div className="flex gap-2">
                  {toolSuggestions.map((suggestion) => (
                    <button
                      ...
                      onClick={() => onToolOpen?.(suggestion.toolName)}
                      className="flex-1 px-3 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-xs text-white transition-all"
                    >
                      <div className="font-medium mb-0.5">{suggestion.toolName}</div>
                      <div className="text-white/60 text-[10px]">{suggestion.reason}</div>
                    </button>
                  ))}
                </div>
              </div>
            ...
            >
              <div className="bg-black/40 rounded-2xl p-2 border border-white/5 shadow-inner">
                <div className="flex items-center gap-2 mb-1 px-2 text-white/70 pt-1">
                  ...
                  {campaignMode && (
                    <span className="ml-auto px-2 py-0.5 bg-purple-500/20 text-purple-400 text-[9px] font-bold uppercase rounded-full">
...
export default EnhancedBottomPromptBar;
